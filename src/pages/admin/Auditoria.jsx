...
import { auditEntityLabel, auditItemFacts, auditSentenceParts, shouldShowAuditItem } from "../../features/audit/descriptions.js";
import { AdminBackButton, AdminReceiptHeader, ExportButtons } from "./shared.jsx";
...
const auditoriaStyles = `
  ...
  .admin-page .timeline { display: grid; gap: .65rem; }
  .admin-page .timeline-item { display: grid; grid-template-columns: 12px minmax(0,1fr); gap: .75rem; border-radius: .9rem; border: 1px solid #e7e5e4; background: #fff; padding: .85rem; }
  .admin-page .timeline-dot { margin-top: .25rem; width: .75rem; height: .75rem; border-radius: 999px; background: #ea580c; }
  .admin-page .timeline-body { display: grid; gap: .32rem; min-width: 0; }
  .admin-page .timeline-body strong { font-size: .92rem; line-height: 1.18; color: #1c1917; }
  .admin-page .timeline-body p { color: #57534e; font-size: .8rem; font-weight: 750; line-height: 1.28; }
  .admin-page .audit-request-link { display: inline; border: 0; border-bottom: 2px solid rgba(234,88,12,.32); background: transparent; padding: 0 .08rem; color: #c2410c; font: inherit; font-weight: 950; text-transform: uppercase; vertical-align: baseline; }
  .admin-page .audit-request-link:hover { border-color: #ea580c; color: #9a3412; transform: none; }
  .admin-page .audit-target-label { color: #c2410c; font-weight: 950; text-transform: uppercase; }
  .admin-page .audit-meta { display: flex; flex-wrap: wrap; gap: .35rem; }
  .admin-page .audit-meta span { border-radius: 999px; border: 1px solid #e7e5e4; background: #fafaf9; padding: .22rem .48rem; color: #57534e; font-size: 10px; font-weight: 900; text-transform: uppercase; }
`;

function AuditSentence({ item, state }) {
  const parts = auditSentenceParts(state, item);
  return (
    <strong>
      {parts.before}
      {parts.target?.requestId ? (
        <button className="audit-request-link" type="button" data-open-request={parts.target.requestId}>{parts.target.label}</button>
      ) : parts.target ? (
        <span className="audit-target-label">{parts.target.label}</span>
      ) : null}
      {parts.after}
    </strong>
  );
            ...
            return (
              <div className="timeline-item" key={item.id}>
                ...
                <div className="timeline-body">
                  <AuditSentence item={item} state={state} />
                  {requestFacts ? <p>{requestFacts}</p> : null}
                  <div className="audit-meta">
                    <span>{formatDateTime(item.at)}</span>
                    <span>{auditEntityLabel(item.entity)}</span>
                  </div>
                </div>
              </div>
            );
