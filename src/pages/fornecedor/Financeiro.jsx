...
export function Financeiro(props) {
  ...
  return (
    <section className="finance-page">
      ...
      />
      <div className="finance-grid"><article className="finance-card"><h2>Composição por refeição</h2>{byMeal.length ? byMeal.map((item) => <div className="finance-progress" key={item.label}><div><span>{item.label}</span><strong>{money(item.value)}</strong></div><i><b style={{ width: `${Math.max(3, Math.round((item.value / max) * 100))}%` }} /></i></div>) : <div className="empty">Sem movimentação no período.</div>}</article><article className="finance-card"><h2>Evolução dos últimos 7 dias</h2><div className="finance-bars">{days.map((item) => <div key={item.key}><strong>{item.value ? money(item.value).replace("R$", "") : "-"}</strong><i style={{ height: `${Math.max(5, Math.round((item.value / dailyMax) * 126))}px` }} /><span>{item.label}</span></div>)}</div></article></div>
      <article className="finance-card finance-table-card">
        ...
        <div className="finance-mobile-movements">
          ...
          {!sortedRows.length && <div className="empty">Nenhuma movimentação encontrada para o período.</div>}
        </div>
